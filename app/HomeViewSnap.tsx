...
export default function HomeViewSnap({messages, locale}: HomeViewProps) {
  ...
  // Open source — every entry is a REAL public repo on github.com/VelkinaStudio.
  const GH = 'https://github.com/VelkinaStudio';
  const repos = [
    { name: 'skill-forge', kind: {en:'Claude Code skill',tr:'Claude Code skill'}, lang: 'JavaScript', url: `${GH}/skill-forge`,
      blurb: {en:"Scaffolds and lints Claude Code skills against Anthropic's authoring rules, so a SKILL.md actually gets discovered and loaded.", tr:"Claude Code skill'lerini Anthropic'in yazım kurallarına göre iskeletliyor ve denetliyor; böylece bir SKILL.md gerçekten keşfedilip yükleniyor."} },
    { name: 'pr-storyteller', kind: {en:'Claude Code skill',tr:'Claude Code skill'}, lang: 'JavaScript', url: `${GH}/pr-storyteller`,
      blurb: {en:"Writes a PR description from the actual diff — grouped by intent, with a risk section and a list of what wasn't tested.", tr:"Bir PR açıklamasını gerçek diff'ten yazar — niyete göre gruplanmış, risk bölümü ve test edilmeyenlerin listesiyle."} },
    { name: 'n8n-flow-auditor', kind: {en:'Agent',tr:'Agent'}, lang: 'Markdown', url: `${GH}/n8n-flow-auditor`,
      blurb: {en:'Reads an exported n8n workflow and names exactly where it will silently fail in production — missing error branches, unauthenticated webhooks, data drift.', tr:'Dışa aktarılmış bir n8n akışını okur ve üretimde sessizce nerede çökeceğini tam olarak söyler — eksik hata dalları, kimliksiz webhooklar, veri kayması.'} },
    { name: 'shopify-liquid-reviewer', kind: {en:'Agent',tr:'Agent'}, lang: 'Markdown', url: `${GH}/shopify-liquid-reviewer`,
      blurb: {en:'Reviews Shopify Liquid themes for the performance, accessibility, and OS 2.0 mistakes that quietly hurt conversion.', tr:'Shopify Liquid temalarını dönüşümü sessizce düşüren performans, erişilebilirlik ve OS 2.0 hataları için inceler.'} },
    { name: 'release-radar', kind: {en:'Workflow',tr:'Workflow'}, lang: 'JavaScript', url: `${GH}/release-radar`,
      blurb: {en:'Turns a git tag range into a categorized changelog, a draft GitHub release, and a short announcement — in one run.', tr:'Bir git etiket aralığını tek seferde kategorize edilmiş bir changeloga, taslak GitHub sürümüne ve kısa bir duyuruya çevirir.'} },
    { name: 'shopify-metafield-mcp', kind: {en:'MCP server',tr:'MCP sunucusu'}, lang: 'JavaScript', url: `${GH}/shopify-metafield-mcp`,
      blurb: {en:"An MCP server that lets Claude read and diff a Shopify store's metafield definitions, so theme work stops guessing namespaces and keys.", tr:"Claude'un bir Shopify mağazasının metafield tanımlarını okuyup karşılaştırmasını sağlayan MCP sunucusu; böylece tema işi namespace ve keyleri tahmin etmeyi bırakır."} },
    { name: 'retry-ladder', kind: {en:'Library',tr:'Kütüphane'}, lang: 'JavaScript', url: `${GH}/retry-ladder`,
      blurb: {en:'A tiny zero-dependency retry library that tells a rate limit from a timeout from a bug, and only retries what is worth retrying.', tr:'Bir rate limiti bir zaman aşımından, bir hatadan ayıran küçük ve bağımsız bir retry kütüphanesi; sadece denemeye değeni dener.'} },
    { name: 'polyglot-parity', kind: {en:'CLI',tr:'CLI'}, lang: 'JavaScript', url: `${GH}/polyglot-parity`,
      blurb: {en:'Fails your build when translation files drift apart — the dropped placeholders and broken plurals that key-counting tools miss.', tr:'Çeviri dosyaları birbirinden kaydığında derlemeni durdurur — anahtar sayan araçların kaçırdığı düşen placeholderlar ve bozuk çoğullar.'} },
    { name: 'utm-lint', kind: {en:'CLI',tr:'CLI'}, lang: 'JavaScript', url: `${GH}/utm-lint`,
      blurb: {en:'Lints ad campaign names and tracking URLs, catching the casing and delimiter mismatches that split one traffic source into five in GA4.', tr:"Reklam kampanya adlarını ve takip URLlerini denetler; GA4'te tek bir trafik kaynağını beşe bölen büyük/küçük harf ve ayraç uyumsuzluklarını yakalar."} },
    { name: 'schema-guard', kind: {en:'CLI',tr:'CLI'}, lang: 'JavaScript', url: `${GH}/schema-guard`,
      blurb: {en:'Extracts every JSON-LD block from your HTML and fails CI when a Product or Article is missing the fields Google needs for rich results.', tr:"HTMLinizdeki her JSON-LD bloğunu çıkarır ve bir Product veya Article, Googleın zengin sonuçlar için istediği alanları kaçırdığında CIı durdurur."} },
    { name: 'licensescan', kind: {en:'CLI',tr:'CLI'}, lang: 'JavaScript', url: `${GH}/licensescan`,
      blurb: {en:"Classifies every installed npm package's license and flags copyleft or unknown risk for your project type, as a CI gate.", tr:'Kurulu her npm paketinin lisansını sınıflandırır ve proje türünüz için copyleft veya bilinmeyen riski bir CI kapısı olarak işaretler.'} },
    { name: 'altpilot', kind: {en:'CLI + library',tr:'CLI + kütüphane'}, lang: 'JavaScript', url: `${GH}/altpilot`,
      blurb: {en:'Audits HTML/JSX/Vue/Svelte files or a live URL for image alt-text accessibility (WCAG 1.1.1), with a CI fail gate.', tr:"HTML/JSX/Vue/Svelte dosyalarını veya canlı bir URLyi görsel alt-metin erişilebilirliği (WCAG 1.1.1) için denetler; CI durdurma kapısıyla."} },
    { name: 'hueshift', kind: {en:'CLI + library',tr:'CLI + kütüphane'}, lang: 'JavaScript', url: `${GH}/hueshift`,
      blurb: {en:'Turns one seed color into a full accessible palette with WCAG contrast guarantees baked in — usable from the CLI or in code.', tr:'Tek bir tohum renkten, WCAG kontrast garantileri yerleşik tam ve erişilebilir bir palet üretir — CLIdan veya kod içinde kullanılır.'} },
    { name: 'inkwell', kind: {en:'Render toolkit',tr:'Render araç seti'}, lang: 'JavaScript', url: `${GH}/inkwell`,
      blurb: {en:'A three.js / React Three Fiber toolkit for the comic-print look — halftone CMYK dots, ink outlines, posterize bands, and stepped on-twos timing.', tr:"Çizgi-roman baskısı görünümü için bir three.js / React Three Fiber araç seti — halftone CMYK noktaları, mürekkep konturları, posterize bantları ve on-twos zamanlama."} },
    { name: 'lumen-theme', kind: {en:'Shopify theme',tr:'Shopify teması'}, lang: 'Liquid', url: `${GH}/lumen-theme`,
      blurb: {en:'A small, dependency-free Shopify Online Store 2.0 theme in clean Liquid and vanilla Web Components — built to be read end to end.', tr:'Temiz Liquid ve sade Web Components ile yazılmış, bağımlılıksız küçük bir Shopify Online Store 2.0 teması — baştan sona okunmak için yapıldı.'} },
  ];
  const ossLabel = lang==='en' ? 'Open source' : 'Açık kaynak';
  const ossIntro = lang==='en'
    ? 'A studio and a lab: tools we built for our own work and put out for anyone. Small, dependency-light, public on GitHub.'
    : 'Bir stüdyo ve bir laboratuvar: kendi işimiz için yapıp herkese açtığımız araçlar. Küçük, az bağımlılıklı, GitHub’da açık.';
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* OPEN SOURCE — real public repos */}
        <section id="opensource" className="snap-start snap-always min-h-[calc(100svh-5rem)] flex items-center">
          <div className="max-w-7xl mx-auto px-6 md:px-10 w-full">
            <div className="flex items-end justify-between mb-2 gap-4 flex-wrap">
              <h2 className="font-heading text-3xl md:text-4xl">
                <span className="vk-chip vk-chip--purple mr-3 align-middle" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><path strokeLinecap="round" strokeLinejoin="round" d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.9a3.4 3.4 0 0 0-.9-2.6c3-.3 6.2-1.5 6.2-6.7A5.2 5.2 0 0 0 20 4.8a4.8 4.8 0 0 0-.1-3.6s-1.1-.3-3.6 1.4a12.3 12.3 0 0 0-6.6 0C7.2.9 6.1 1.2 6.1 1.2A4.8 4.8 0 0 0 6 4.8a5.2 5.2 0 0 0-1.4 3.7c0 5.2 3.2 6.4 6.2 6.7a3.4 3.4 0 0 0-.9 2.6V22"/></svg>
                </span>
                {ossLabel}
              </h2>
              <a href="https://github.com/VelkinaStudio" target="_blank" rel="noopener noreferrer" className="text-white/80 hover:text-vkcyan text-sm border border-white/15 rounded-lg px-3 py-1.5 bg-white/5 hover:bg-white/10">
                {lang==='en' ? 'All repositories ↗' : 'Tüm repolar ↗'}
              </a>
            </div>
            <p className="text-white/70 max-w-2xl mb-6">{ossIntro}</p>

            <div className="relative" data-rail="opensource" data-auto="true" data-interval="3600" tabIndex={0}>
              <div className="pointer-events-none absolute inset-y-0 left-0 w-16 z-10" style={{background:'linear-gradient(90deg, var(--vk-bg), transparent)'}}></div>
              <div className="pointer-events-none absolute inset-y-0 right-0 w-16 z-10" style={{background:'linear-gradient(270deg, var(--vk-bg), transparent)'}}></div>
              <button className="vk-arrow left-2 inline-flex" data-rail-left="opensource" aria-label={lang==='en' ? 'Scroll left' : 'Sola kaydır'}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5"/></svg>
              </button>
              <button className="vk-arrow right-2 inline-flex" data-rail-right="opensource" aria-label={lang==='en' ? 'Scroll right' : 'Sağa kaydır'}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5 15.75 12l-7.5 7.5"/></svg>
              </button>
              <div className="overflow-x-auto hide-scrollbar" data-rail-scroller>
                <div className="flex gap-4 pr-8 snap-x snap-mandatory" role="list">
                  {repos.map((r, i) => (
                    <a key={r.name} href={r.url} target="_blank" rel="noopener noreferrer" role="listitem" className="min-w-[280px] max-w-[320px] snap-start vk-glass border border-white/10 rounded-xl p-5 shadow-soft hover:shadow-strong hover:-translate-y-0.5 transition block focus:outline-none focus:ring-2 focus:ring-vkcyan/50">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-mono text-xs px-2 py-0.5 rounded-md border border-white/15 text-white/70">{r.kind[lang as 'en'|'tr']}</span>
                        <span className="inline-flex items-center gap-1.5 text-xs text-white/55"><span className="w-2 h-2 rounded-full" style={{background: r.lang==='Liquid' ? '#7C3AED' : r.lang==='Markdown' ? '#00FFFF' : '#FF00CC'}}></span>{r.lang}</span>
                      </div>
                      <h3 className="font-heading text-lg text-white/90 break-words">{r.name}</h3>
                      <p className="text-white/70 text-sm mt-2">{r.blurb[lang as 'en'|'tr']}</p>
                      <div className="mt-3 inline-flex items-center text-white/80 text-sm">{lang==='en' ? 'View on GitHub' : "GitHub'da gör"}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-4 h-4 ml-1" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12l-7.5 7.5M21 12H3"/></svg></div>
                    </a>
                  ))}
                </div>
              </div>
