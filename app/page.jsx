  ...
  return (
    <div>
      ...
      <section className="vk-hero relative overflow-hidden min-h-[86vh]">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-10 pt-28 pb-20 grid gap-10 sm:grid-cols-2 items-center">
          ...
          <div>
            <h1 className="font-heading text-4xl md:text-6xl leading-tight">
              <span id="vk-hero-steps-text" className="inline-block align-baseline" aria-live="polite" aria-atomic="true">{t?.heroSteps?.[0] ?? 'Keşfet'}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3 pt-4">
              ...
              <a data-cta="email" className="inline-flex items-center px-6 py-3 rounded-xl border border-vkcyan/50 text-vkcyan/90 bg-white/5 hover:bg-white/10 transition">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5 mr-2" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6A2.25 2.25 0 0 1 19.5  4.5h-15a2.25 2.25 0  0 1-2.25 2.25v15a2.25 2.25 0  0 1 2.25 2.25h15a2.25 2.25 0  0 1 2.25-2.25v-15ZM3 18.75V7.5a2.25 2.25 0  0 1 2.25-2.25h13.5A2.25 2.25 0  0 1 21 7.5v11.25m-18 0A2.25 2.25 0  0 0 3 18.75m15.686 0A11.953 11.953 0  0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0  0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0  0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0  0 1 3 12c0-1.605.42-3.113 1.157-4.418"/></svg>
        ...
        <HeroStepsClient steps={t?.heroSteps} />
        <HeroShapesClient />
      </section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" role="list" aria-label="Featured services">
          ...
          <article role="listitem" className="vk-card vk-glass border border-white/10 rounded-xl p-5 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-xl">{t?.serviceCards?.web?.title ?? 'Web Geliştirme'}</h3>
            </div>
            <p className="text-white/80">{t?.serviceCards?.web?.desc ?? 'Next.js 14 ile premium hareket ve sağlam SEO temellerine sahip web siteleri ve uygulamalar.'}</p>
            <a href={`${locale ? '/' + locale : ''}/services#web`} aria-label={t?.serviceCards?.web?.aria ?? "Web Geliştirme'yi inceleyin"} className="mt-3 inline-flex items-center text-vkcyan hover:text-white/90">
              {t?.explore ?? 'İncele'}
          ...
          <article role="listitem" className="vk-card vk-glass border border-white/10 rounded-xl p-5 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-xl">{t?.serviceCards?.edge?.title ?? 'Edge Hosting & DevOps'}</h3>
            </div>
            <p className="text-white/80">{t?.serviceCards?.edge?.desc ?? 'Önizlemeler, geri almalar ve Vercel/Cloudflare üzerinde edge hız ile sakin sürümler.'}</p>
            <a href={`${locale ? '/' + locale : ''}/services#hosting`} aria-label={t?.serviceCards?.edge?.aria ?? "Edge Hosting & DevOps'u inceleyin"} className="mt-3 inline-flex items-center text-vkcyan hover:text-white/90">
              {t?.explore ?? 'İncele'}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-4 h-4 ml-1" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12l-7.5 7.5M21 12H3"/></svg>
            </a>
          </article>
          ...
          <article role="listitem" className="vk-card vk-glass border border-white/10 rounded-xl p-5 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-xl">{t?.serviceCards?.ai?.title ?? 'Yapay Zekâ Asistanları & Otomasyon'}</h3>
            </div>
            <p className="text-white/80">{t?.serviceCards?.ai?.desc ?? 'Operasyon, destek ve analitik için gerçekten çalışan ve sürdürülebilir asistanlar.'}</p>
            <a href={`${locale ? '/' + locale : ''}/services#production`} aria-label={t?.serviceCards?.ai?.aria ?? "Yapay Zekâ Asistanları & Otomasyon'u inceleyin"} className="mt-3 inline-flex items-center text-vkcyan hover:text-white/90">
              {t?.explore ?? 'İncele'}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-4 h-4 ml-1" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12l-7.5 7.5M21 12H3"/></svg>
            </a>
          </article>
          ...
          <article role="listitem" className="vk-card vk-glass border border-white/10 rounded-xl p-5 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-xl">{t?.serviceCards?.middleware?.title ?? 'CRM–CMS Orta Katman'}</h3>
            </div>
            <a href={`${locale ? '/' + locale : ''}/services#middleware`} aria-label={t?.serviceCards?.middleware?.aria ?? "CRM–CMS Orta Katman'ı inceleyin"} className="mt-3 inline-flex items-center text-vkcyan hover:text-white/90">
              {t?.explore ?? 'İncele'}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-4 h-4 ml-1" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12l-7.5 7.5M21 12H3"/></svg>
            </a>
          </article>
    ...
    <CardParallaxClient />
      ...
      <section aria-labelledby="results" className="max-w-7xl mx-auto px-6 md:px-10 py-12">
        <div className="flex items-end justify-between mb-6">
          <h2 id="results" className="font-heading text-2xl md:text-3xl">{t?.resultsTitle ?? 'Kanıtlanmış Sonuçlar'}</h2>
          <span className="hidden sm:inline text-white/60 text-sm">{t?.resultsSubtitle ?? 'Son işlerden öne çıkanlar'}</span>
        </div>
        {/* Metrics */}
        <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-6 mb-6">
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.published?.sr ?? '120+ yayınlanan web sitesi ve uygulama'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.published?.label ?? 'Yayınlanan web sitesi ve uygulama'}</div>
          </div>
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.cwv?.sr ?? 'Sayfaların yüzde 86’sı Core Web Vitals “İyi” seviyesine taşındı'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.cwv?.label ?? 'Core Web Vitals “İyi”ye taşınan sayfalar'}</div>
          </div>
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.conversion?.sr ?? 'Lansman sonrası ortalama dönüşüm artışı yüzde 35'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.conversion?.label ?? 'Lansman sonrası ortalama dönüşüm artışı'}</div>
          </div>
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.medianTime?.sr ?? 'Medyan yayına alma süresi 5 haftadır'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.medianTime?.label ?? 'Medyan yayına alma süresi'}</div>
          </div>
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.integrations?.sr ?? '40+ API ve entegrasyon otomatikleştirildi'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.integrations?.label ?? 'Otomatikleştirilen API ve entegrasyon'}</div>
          </div>
          <div className="vk-glass border border-white/10 rounded-xl p-4 shadow-soft text-center">
            ...
            <span className="sr-only">{t?.metrics?.uptime?.sr ?? 'Gözlemlenen edge çalışma süresi yüzde 99.9'}</span>
            <div className="text-white/70 text-sm">{t?.metrics?.uptime?.label ?? 'Gözlemlenen edge çalışma süresi'}</div>
          </div>
        </div>
      ...
      <section id="why" className="vk-section--values max-w-7xl mx-auto px-6 md:px-10 py-16">
        <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
          <div>
            <h2 className="font-heading text-3xl md:text-4xl">{t?.why?.title ?? 'Neden Velkina'}</h2>
            <p className="text-white/80 max-w-3xl mt-1">{t?.why?.subtitle ?? 'Kıdemli ürün, tasarım ve mühendislik tek şeritte. Premium marka deneyimleri ve güvenilir sistemleri hızlı, ölçülebilir ve sakin bir şekilde hayata geçiriyoruz.'}</p>
          </div>
        ...
        <div className="grid gap-6 grid-cols-1 lg:grid-cols-12">
          ...
          <article className="vk-glass border border-white/10 rounded-xl p-6 shadow-soft reveal-on-scroll lg:col-span-5 lg:row-span-2">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="font-heading text-lg">{t?.why?.howWeWork?.title ?? 'Nasıl çalışıyoruz'}</h3>
            </div>
            <ul className="space-y-2 text-white/80">
              <li className="flex items-start gap-2"><span className="mt-0.5 inline-block w-1.5 h-1.5 rounded-full bg-vkcyan" aria-hidden="true"></span><span>{t?.why?.howWeWork?.items?.[0] ?? 'Önizlemeler, geri almalar ve net demolarla haftalık gönderim ritmi.'}</span></li>
              <li className="flex items-start gap-2"><span className="mt-0.5 inline-block w-1.5 h-1.5 rounded-full bg-vkpink" aria-hidden="true"></span><span>{t?.why?.howWeWork?.items?.[1] ?? 'Önce tasarım sistemi: tutarlılık için token’lar, bileşenler ve hareket.'}</span></li>
              <li className="flex items-start gap-2"><span className="mt-0.5 inline-block w-1.5 h-1.5 rounded-full bg-white/60" aria-hidden="true"></span><span>{t?.why?.howWeWork?.items?.[2] ?? 'Ölçülebilir büyüme: sunucu tarafı analitik, deneyler ve CRO.'}</span></li>
            </ul>
            <div className="mt-4 grid grid-cols-3 gap-3 text-center" aria-hidden="true">
              <div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="font-heading">120+</div><div className="text-xs text-white/70">{t?.why?.howWeWork?.stats?.launches ?? 'Lansman'}</div></div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="font-heading">5</div><div className="text-xs text-white/70">{t?.why?.howWeWork?.stats?.median ?? 'Medyan süre'}</div></div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="font-heading">99.9%</div><div className="text-xs text-white/70">{t?.why?.howWeWork?.stats?.uptime ?? 'Edge çalışma süresi'}</div></div>
            </div>
          </article>
          ...
          <article className="vk-card vk-glass border border-white/10 rounded-xl p-6 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll lg:col-span-7">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-xl">{t?.why?.seniorOnly?.title ?? 'Sadece kıdemli ekip'}</h3>
            </div>
            <p className="text-white/80">{t?.why?.seniorOnly?.desc ?? 'Handover yok, yük yok. Strateji, tasarım ve kod birlikte ilerler; siz de öyle.'}</p>
          </article>
          ...
          <article className="vk-card vk-glass border border-white/10 rounded-xl p-6 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll lg:col-span-3">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-lg">{t?.why?.speed?.title ?? 'Stres olmadan hız'}</h3>
            </div>
            <p className="text-white/80">{t?.why?.speed?.desc ?? 'Edge barındırma, CI/CD ve tip güvenli API’ler. Önizlemeler her adımı güvenli kılar.'}</p>
          </article>
          ...
          <article className="vk-card vk-glass border border-white/10 rounded-xl p-6 shadow-soft hover:shadow-strong hover:-translate-y-1 hover:scale-[1.01] transition reveal-on-scroll lg:col-span-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-heading text-lg">{t?.why?.design?.title ?? 'Dönüştüren tasarım'}</h3>
            </div>
            <p className="text-white/80">{t?.why?.design?.desc ?? 'Premium hareket ve net hikâye. Sadece güzel görünen değil, satan sistemler.'}</p>
          </article>
        </div>
      </section>
